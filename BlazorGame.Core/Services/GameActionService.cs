...
public class GameActionService
{
    ...
    private (GameActionResult result, int points, int health) DetermineActionResult(ActionType actionType, in GameRewards config)
    {
        return actionType switch
        {
            ActionType.Combat => DetermineCombatResult(config),
            ActionType.Search => DetermineSearchResult(config),
            ActionType.Flee => (GameActionResult.Escaped, _rewardsService.GetRandomInRange(config.MinFleePoints, config.MaxFleePoints), 0),
    ...
    private (GameActionResult result, int points, int health) DetermineCombatResult(in GameRewards config)
    {
        ...
        {
            return (GameActionResult.Victory, _rewardsService.GetRandomInRange(config.MinCombatVictoryPoints, config.MaxCombatVictoryPoints), 0);
        }
    ...
    private (GameActionResult result, int points, int health) DetermineSearchResult(in GameRewards config)
    {
        ...
        if (roll < 33)
        {
            return (GameActionResult.FoundTreasure, _rewardsService.GetRandomInRange(config.MinTreasurePoints, config.MaxTreasurePoints), 0);
        }
        
        if (roll < 66)
        {
            return (GameActionResult.FoundPotion, 0, _rewardsService.GetRandomInRange(config.MinPotionHealthGain, config.MaxPotionHealthGain));
        }
